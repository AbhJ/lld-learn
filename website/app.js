...
(function () {
	...
	// Load saved edits from localStorage on boot
	function loadEditBuffers() {
		try {
			const stored = localStorage.getItem('lld-edit-buffers');
			if (stored) {
				const obj = JSON.parse(stored);
				Object.entries(obj).forEach(([k, v]) => editBuffers.set(k, v));
			}
		} catch (e) { /* ignore */ }
	}

	// Persist all edit buffers to localStorage
	function saveEditBuffers() {
		const obj = {};
		editBuffers.forEach((v, k) => { obj[k] = v; });
		localStorage.setItem('lld-edit-buffers', JSON.stringify(obj));
	}

	function updateSaveButton() {
		if (editBuffers.size > 0) {
			dom.btnSave.style.display = '';
			dom.btnSave.querySelector('span').textContent = `Save (${editBuffers.size})`;
		} else {
	...
	function updateResetButton() {
		if (currentProblemId && currentFilePath) {
			const key = bufferKey(currentProblemId, currentFilePath);
			dom.btnReset.style.display = editBuffers.has(key) ? '' : 'none';
		} else {
	...
	function renderFolderTree() {
		...
		// Arrow click → expand/collapse file tree (no navigation)
		dom.folderTree.querySelectorAll('.tree-arrow').forEach(el => {
			el.addEventListener('click', (e) => {
				...
				if (expandedProblemId === id) {
					...
					if (expandedProblemId) collapseProblemInSidebar(expandedProblemId);
					expandedProblemId = id;
					const filesEl = document.getElementById('tree-files-' + id);
		...
		// Problem name click → always navigate to description page
		dom.folderTree.querySelectorAll('.tree-problem-name').forEach(el => {
			el.addEventListener('click', (e) => {
				e.stopPropagation();
				const header = el.closest('.tree-problem-header');
				const id = header.dataset.id;
		...
		// Header click (anywhere else on the row) → navigate to description
	...
	function showProblem(problem) {
		...
		dom.viewTips.style.display = 'none';
	...
	function showCode(problem, filePath) {
		...
		// Code content — use edited version if available
		const editKey = bufferKey(problem.id, filePath);
		dom.codeContent.textContent = editBuffers.get(editKey) ?? file.content;
	...
	function handleRoute() {
		...
		if (hash === 'patterns') {
			...
			expandedProblemId = null;
			showWelcome();
			// Reset filters and show pattern chips prominently
	...
	async function showTips() {
		...
		dom.viewProblem.style.display = 'none';
		dom.viewCode.style.display = 'none';
		dom.viewTips.style.display = 'block';

		if (tipsLoaded) {
			buildTOC(dom.tipsBody);
			return;
		}
		try {
			const resp = await fetch('tips.md');
			if (!resp.ok) throw new Error('tips.md not found');
			const md = await resp.text();
			dom.tipsBody.innerHTML = renderMarkdown(md);
			if (window.Prism) Prism.highlightAllUnder(dom.tipsBody);
			tipsLoaded = true;
			buildTOC(dom.tipsBody);
		} catch (err) {
			dom.tipsBody.innerHTML =
				'<p class="tips-error">Could not load interview tips. Make sure <code>tips.md</code> is present in the website folder.</p>';
		}
	}

	function updateActiveNav(navName) {
		document.querySelectorAll('.nav-link').forEach(l => {
			l.classList.toggle('active', l.dataset.nav === navName);
		});
	}
	...
	function setupListeners() {
		// Support section toggle (lazy-load iframe)
		const supportToggle = document.getElementById('support-toggle');
		const supportEmbed = document.getElementById('support-embed');
		const bmcFrame = document.getElementById('bmc-frame');
		if (supportToggle && supportEmbed && bmcFrame) {
			supportToggle.addEventListener('click', () => {
				const opening = !supportEmbed.classList.contains('open');
				supportEmbed.classList.toggle('open');
				// Lazy-load: set src on first open
				if (opening && bmcFrame.src === 'about:blank') {
					bmcFrame.src = bmcFrame.dataset.src;
				}
			});
		}
		...
		dom.btnCopy.addEventListener('click', () => {
			...
			const header = '// Copyright (c) 2026 abj. All rights reserved.\n// Source: https://abhj.github.io\n\n';
		...
		// Edit / Save / Reset / Run / Tests / Run-panel-close
		dom.btnEdit.addEventListener('click', toggleEditMode);
		dom.btnSave.addEventListener('click', () => {
			saveEditBuffers();
			dom.btnSave.querySelector('span').textContent = 'Saved!';
	...
	function exitEditMode() {
		...
		// Refresh code display with edited content
		if (currentProblemId && currentFilePath) {
			const key = bufferKey(currentProblemId, currentFilePath);
			if (editBuffers.has(key)) {
				dom.codeContent.textContent = editBuffers.get(key);
				dom.codeContent.className = 'language-java';
				Prism.highlightElement(dom.codeContent);
			}
		}
	...
	async function runTests() {
		...
		for (let i = 0; i < cases.length; i++) {
			...
			row.innerHTML = `<div class="test-case-header">
				<div class="test-case-status skip">…</div>
				<div class="test-case-name">${tc.name || 'Case ' + (i+1)}</div>
			</div>`;
			container.appendChild(row);
			const result = await runOneTestCase(problem, variant, tc);
			const statusEl = row.querySelector('.test-case-status');
			...
			// Build detail grid with input/expected/actual
			const detail = document.createElement('div');
			detail.className = 'test-case-body' + (passed ? '' : ' test-case-body--fail');

			// Input
			if (tc.stdin) {
				detail.innerHTML += `<div class="test-io-row">
					<span class="test-io-label">Input</span>
					<pre class="test-io-value">${escapeHtml(tc.stdin.slice(0, 500))}</pre>
				</div>`;
			}

			// Expected
			const expected = tc.expectedStdoutEquals
				|| (tc.expectedStdoutContains ? 'Contains: ' + tc.expectedStdoutContains.join(', ') : null)
				|| (tc.expectedExitCode !== undefined ? 'Exit code: ' + tc.expectedExitCode : null);
			if (expected) {
				detail.innerHTML += `<div class="test-io-row">
					<span class="test-io-label">Expected</span>
					<pre class="test-io-value">${escapeHtml(String(expected).slice(0, 500))}</pre>
				</div>`;
			}
			...
			if (result.stdout) {
				detail.innerHTML += `<div class="test-io-row">
					<span class="test-io-label">Output</span>
					<pre class="test-io-value">${escapeHtml(result.stdout.slice(0, 800))}</pre>
				</div>`;
			}

			// Stderr (only on failure)
			if (!passed && result.stderr) {
				detail.innerHTML += `<div class="test-io-row test-io-row--error">
					<span class="test-io-label">Stderr</span>
					<pre class="test-io-value">${escapeHtml(result.stderr.slice(0, 500))}</pre>
				</div>`;
			}
			...
			if (!passed && result.reason) {
				detail.innerHTML += `<div class="test-io-row test-io-row--error">
					<span class="test-io-label">Reason</span>
					<pre class="test-io-value">${escapeHtml(result.reason)}</pre>
				</div>`;
			}
	...
	/**
	 * Build and display a Table of Contents from h2/h3 headings inside containerEl.
	 */
	// Store headings array for scroll-spy index-based matching
	let tocHeadings = [];

	function buildTOC(containerEl) {
		...
		// Assign unique IDs using index to avoid duplicates
		...
		tocHeadings.forEach((heading, i) => {
			...
			const text = rawText;
			const safeTitle = rawText.replace(/"/g, '&quot;').replace(/</g, '&lt;');
			linksHtml += `<a class="toc-link toc-${level}" data-toc-idx="${i}" title="${safeTitle}">${escapeHtml(text)}</a>`;
		});

		dom.tocNav.innerHTML = linksHtml;
		dom.tocSidebar.classList.add('visible');
		dom.mainContent.classList.add('has-toc');

		// Click handlers
		dom.tocNav.querySelectorAll('.toc-link').forEach(link => {
			link.addEventListener('click', (e) => {
				e.preventDefault();
				const idx = parseInt(link.dataset.tocIdx, 10);
				const targetEl = tocHeadings[idx];
				if (!targetEl) return;

				const containerRect = dom.mainContent.getBoundingClientRect();
				const headingRect = targetEl.getBoundingClientRect();
				const scrollOffset = headingRect.top - containerRect.top + dom.mainContent.scrollTop - 24;
				dom.mainContent.scrollTo({ top: scrollOffset, behavior: 'smooth' });

				// Flash highlight
				targetEl.classList.add('toc-highlight');
				setTimeout(() => targetEl.classList.remove('toc-highlight'), 1500);

				// Immediately mark active and lock scroll-spy
				lockScrollSpy();
				dom.tocNav.querySelectorAll('.toc-link').forEach(l => l.classList.remove('active'));
	...
	function hideTOC() {
		dom.tocSidebar.classList.remove('visible');
		dom.mainContent.classList.remove('has-toc');
		dom.tocNav.innerHTML = '';
		tocHeadings = [];
		detachScrollSpy();
	}

	/** Scroll-spy using index-based matching — no ID collisions possible */
	...
	function lockScrollSpy() {
		...
		scrollSpyLockTimer = setTimeout(() => { scrollSpyLocked = false; }, 1000);
	}

	function attachScrollSpy() {
		detachScrollSpy();
		const tocLinks = Array.from(dom.tocNav.querySelectorAll('.toc-link'));
		if (!tocLinks.length) return;

		scrollSpyHandler = function () {
			if (scrollSpyRAF || scrollSpyLocked) return;
			scrollSpyRAF = requestAnimationFrame(() => {
				scrollSpyRAF = null;
				if (!tocHeadings.length || scrollSpyLocked) return;

				const threshold = dom.mainContent.getBoundingClientRect().top + 80;
				let activeIdx = 0;

				// If scrolled to bottom, highlight the last heading
				const atBottom = dom.mainContent.scrollHeight - dom.mainContent.scrollTop - dom.mainContent.clientHeight < 30;
				if (atBottom) {
					activeIdx = tocHeadings.length - 1;
				} else {
					for (let i = 0; i < tocHeadings.length; i++) {
						if (tocHeadings[i].getBoundingClientRect().top <= threshold) {
							activeIdx = i;
						} else {
							break;
						}
				...
				if (activeLink) {
					const sidebarRect = dom.tocSidebar.getBoundingClientRect();
					const linkRect = activeLink.getBoundingClientRect();
					if (linkRect.top < sidebarRect.top + 50 || linkRect.bottom > sidebarRect.bottom - 30) {
						activeLink.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
					}
				}
			});
		};
		dom.mainContent.addEventListener('scroll', scrollSpyHandler, { passive: true });
		scrollSpyHandler();
	}

	function detachScrollSpy() {
		if (scrollSpyHandler) {
			dom.mainContent.removeEventListener('scroll', scrollSpyHandler);
